...
if grep -Eiq '\\.(ipa|mobileprovision|p8|p12|cer|iso|gcm|rvz|wbfs|wia|gcz|raw|sav)$' <<<"$public_files"; then
...
matches="$(
  list_public_files |
    xargs -0 grep -IEn \
    '(/Us[e]rs/[^ /]+/|-----BE[G]IN (RSA |EC |OPENSSH )?PRIVATE KEY|github_pat_[A-Za-z0-9_]+|ghp_[A-Za-z0-9]+|sk-[A-Za-z0-9]{20,}|00008[0-9A-F]{3}-[0-9A-F]{16})' \
    2>/dev/null || true
)"
if [[ -n "$matches" ]]; then
  printf '%s\n' "$matches" >&2
  fail "tracked text contains a local path, private credential, or device identifier"
fi
...
printf '[public audit] PASS: no tracked packages, signing assets, game images, saves, local paths, private-key markers, or device IDs\n'
